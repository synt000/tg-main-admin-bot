...
class ShopService:
    ...
    def create_enterprise_order(biz_id, prod_id, customer_id, buy_count, payment_method="KBZPay"):
        ...
        total_amount = float(price * buy_count)
        ...
        # 🔗 🌐 [INTEGRATION HOOK]: အစ်ကို ညွှန်ကြားထားသည့် ဗဟို CRM Event Layer ဆီသို့ အော်တို တွန်းပို့ခြင်း
        try:
            CRMEvents.record_purchase(biz_id, int(customer_id), "shop", total_amount)
        except Exception as ex:
            print(f"CRM Sync Hook Logged Warning: {ex}")
    ...
    def track_order_status(biz_id, order_id):
        ...
        return {"order_id": order[0], "business_id": order[1], "customer_id": order[2], "total_amount": order[3], "status": order[4]}
    ...
    def get_shop_analytics(biz_id):
        ...
        sales = cur.fetchone(); t_sales = sales[0] if sales and isinstance(sales, tuple) else 0
        cur.execute("SELECT COUNT(*) as total_orders FROM orders WHERE business_id = %s;", (biz_id,))
        orders = cur.fetchone(); t_orders = orders[0] if orders and isinstance(orders, tuple) else 0
        cur.close(); conn.close()
